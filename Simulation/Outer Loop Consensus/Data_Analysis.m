...
p1_r = my_logsout.get('p1_r');
p1r_data = p1_r.Values.Data;

pos2 = my_logsout.get('pos2');
pos2_data = pos2.Values.Data;
p2_r = my_logsout.get('p2_r');
p2r_data = p2_r.Values.Data;

pos3 = my_logsout.get('pos3');
pos3_data = pos3.Values.Data;
p3_r = my_logsout.get('p3_r');
p3r_data = p3_r.Values.Data;

pos4 = my_logsout.get('pos4');
pos4_data = pos4.Values.Data;
p4_r = my_logsout.get('p4_r');
p4r_data = p4_r.Values.Data;

pos5 = my_logsout.get('pos5');
pos5_data = pos5.Values.Data;
p5_r = my_logsout.get('p5_r');
p5r_data = p5_r.Values.Data;

pos = {pos1_data, pos2_data, pos3_data, pos4_data, pos5_data};
pos_es = {p1r_data, p2r_data, p3r_data, p4r_data, p5r_data};
...
ex_es = zeros(5,length(time));
ey_es = zeros(5,length(time));
ez_es = zeros(5,length(time));

for i = 1:length(time)
    for j = 1:n_drones
        ex(j,i) = abs(pos{j}(i,1) - pos_ref{j}(1,:,i));
        ey(j,i) = abs(pos{j}(i,2) - pos_ref{j}(2,:,i));
        ez(j,i) = abs(pos{j}(i,3) - pos_ref{j}(3,:,i));

        ex_es(j,i) = abs(pos_es{j}(1,:,i) - pos_ref{j}(1,:,i));
        ey_es(j,i) = abs(pos_es{j}(2,:,i) - pos_ref{j}(2,:,i));
        ez_es(j,i) = abs(pos_es{j}(3,:,i) - pos_ref{j}(3,:,i));
    end
end
...
for i = 1:n_drones
    plot(time, ex_es(i,:), 'Color', colors(i,:), 'LineWidth', 1.5);
end
xlabel('Time (s)', 'FontSize', 11);
ylabel('Error (m)', 'FontSize', 12);
title('Error X of 5 drones (Outer Loop)', 'FontSize', 12);
legend(arrayfun(@(x) sprintf('Drone %d', x), 1:n_drones, 'UniformOutput',false));
...
for i = 1:n_drones
    plot(time, ey_es(i,:), 'Color', colors(i,:), 'LineWidth', 1.5);
end
xlabel('Time (s)', 'FontSize', 11);
ylabel('Error (m)', 'FontSize', 12);
title('Error Y of 5 drones (Outer Loop)', 'FontSize', 12);
legend(arrayfun(@(x) sprintf('Drone %d', x), 1:n_drones, 'UniformOutput',false));
...
for i = 1:n_drones
    plot(time, ez_es(i,:), 'Color', colors(i,:), 'LineWidth', 1.5);
end
xlabel('Time (s)', 'FontSize', 11);
ylabel('Error (m)', 'FontSize', 12);
title('Error Z of 5 drones (Outer Loop)', 'FontSize', 12);
legend(arrayfun(@(x) sprintf('Drone %d', x), 1:n_drones, 'UniformOutput',false));